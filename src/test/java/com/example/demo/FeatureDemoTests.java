...
class FeatureDemoTests {
	...
	void limitsTomcatToOnePlatformWorkerThread() {
		assertThat(environment.getProperty("server.tomcat.threads.max", Integer.class)).isEqualTo(1);
		assertThat(environment.getProperty("server.tomcat.threads.min-spare", Integer.class)).isEqualTo(1);
	}
	...
	void exposesOnlyTheBlockingBenchmarkEndpoint() throws Exception {
		...
		HttpRequest removedEndpoint = HttpRequest.newBuilder(URI.create("http://localhost:" + port + "/thread"))
			.GET()
			.build();
		assertThat(HttpClient.newHttpClient().send(removedEndpoint, HttpResponse.BodyHandlers.ofString()).statusCode())
			.isEqualTo(404);
	}
